...
results_dir = os.path.join(base_dir, "results", "04_lstm_bidirectional_dropout_batchnorm")
...
# ----- Model -----
model = Sequential([
    Bidirectional(LSTM(units=64, return_sequences=False), input_shape=X.shape[1:]),
    BatchNormalization(),
    Dropout(0.5),
...
plt.title("Bidirectional LSTM + Dropout + BatchNorm - Confusion Matrix")
plt.tight_layout()
