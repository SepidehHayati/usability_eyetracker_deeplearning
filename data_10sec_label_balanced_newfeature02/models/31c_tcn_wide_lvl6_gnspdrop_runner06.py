# models/31c_tcn_wide_lvl6_gnspdrop_runner06.py
# TCN (channels=96, kernel=7, levels=6) + GroupNorm + SpatialDropout1D + GELU
# نسخهٔ C: عمیق‌تر از مدل 29 (levels=6) با تنظیمات آموزشی قوی‌تر.
...
# --- تنظیمات آموزشی پیشنهادی برای مدل عمیق‌تر ---
CONFIG = {
    "epochs": 40,
    "batch_size": 32,
    "lr": 1e-3,
...
def same_length_padding(kernel_size, dilation):
    # برای conv با stride=1 طول خروجی از ورودی بزرگ‌تر می‌شود؛ بعداً crop می‌کنیم
    return (kernel_size - 1) * dilation

class SpatialDropout1D(nn.Module):
    """کانال‌-دراپ‌اوت برای ورودی (B, C, T)؛ شبیه SpatialDropout1D در Keras."""
...
class GN(nn.Module):
    """GroupNorm پایدار برای batchهای کوچک."""
...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        # conv1 (خروجی را به طول ورودی crop می‌کنیم)
        out = self.conv1(x)
        out = out[..., :x.size(-1)]
...
class TCN(nn.Module):
    def __init__(self, in_ch=16, channels=96, levels=6, kernel_size=7, dropout=0.2):
        super().__init__()
        layers = []
        c_in = in_ch
...
def build_model(input_channels=16, seq_len=1500):
    # فقط levels = 6 نسبت به مدل 29 تغییر دارد؛ سایر هایپرها همان ترکیب برندهٔ قبلی
    return TCN(in_ch=input_channels,
               channels=96,
               levels=6,          # ← از 5 به 6
               kernel_size=7,     # (اختیاری برای تست بعدی: 5)
