# 22_ensemble_logitavg_bacc.py
# Ensemble با:
# 1) وزن‌دهی دقیق (grid با گام 0.1 روی وزن‌ها، نرمال‌سازی به جمع=1، وزن صفر مجاز)
# 2) ترکیب به روش logit-averaging (نه میانگین احتمال)
# 3) قیود HARD/SOFT و fallback = بیشینه Balanced Accuracy
# 4) کلید انتخاب وزن: (Acc, BAcc, F1, Rec, Prec)
...
# مدل‌های داخل انسمبل (می‌توانی تغییر دهی/کم و زیاد کنی)
MODEL_RESULTS = [
...
# شبکه‌ی آستانه‌ها (اگر کند شد 301 بگذار)
...
EPS = 1e-6  # برای جلوگیری از inf در logit
...
def choose_thr_accfirst_vec(y_true: np.ndarray, y_prob: np.ndarray, thrs: np.ndarray = THRS):
    ...
    # 3) Fallback: **max Balanced Accuracy** (تساوی → Accuracy بالاتر → بعد Precision)
...
def _gen_weight_grid_fine(M: int, step: float = 0.1):
    """شبکه‌ی وزن پیوسته (گام=step). خروجی: آرایه‌های وزن نرمال‌شده (جمع=1)، وزن صفر مجاز."""
    ...
    for w in itertools.product(vals, repeat=M):
        ...
        seen.add(key)
        weights.append(w)
    return weights  # تعداد ~ (11^M -1) با حذف تکراری‌های نرمال‌شده

def main():
    # 1) لود احتمال‌های هر مدل برای هر فولد
    folds_data = {}
    for k in range(1, 6):
        probs = []
        yref = None
    ...
    out_root = os.path.join(RESULTS_DIR, "22_ensemble_logitavg_bacc")
    ...
    for k in range(1, 6):
        ...
        for w in weight_grids:
            ...
            # انتخاب وزن: Acc → BAcc → F1 → Rec → Prec
        ...
        best_weights_per_fold[k] = np.round(w_best, 3).tolist()
        ...
        with open(os.path.join(out_fold, "metrics_ens.json"), "w") as f:
            json.dump({
                "acc": mb["acc"], "prec": mb["prec"], "rec": mb["rec"], "f1": mb["f1"], "thr": mb["thr"], "bacc": mb["bacc"],
                "hard_constraints_satisfied": bool(hard_ok),
                "soft_constraints_used": bool(soft_ok),
                "weights": best_weights_per_fold[k],
    ...
    with open(os.path.join(out_root, "_summary_avg_std.json"), "w") as f:
        json.dump({
            ...
            "note": "Logit-averaging ensemble; fallback=max BAcc; weight key=(Acc,BAcc,F1,Rec,Prec); fine weight grid",
            "models": MODEL_RESULTS,
