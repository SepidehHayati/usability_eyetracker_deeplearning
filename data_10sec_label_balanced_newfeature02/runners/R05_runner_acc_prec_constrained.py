# -*- coding: utf-8 -*-
"""
R05_runner_acc_prec_constrained.py
- 5-fold CV with train-only scaling
- EarlyStopping on F1@0.5 (مثل قبل)
- Threshold tuning: maximize Accuracy subject to F1 >= MIN_F1 and Precision >= MIN_PREC
- Reads weight_decay and pos_weight policy from model CONFIG
"""
...
# -------- constraints for threshold tuning --------
MIN_F1 = 0.75
MIN_PREC = 0.75
...
for _root in CANDIDATE_ROOTS:
    if all(os.path.isdir(os.path.join(_root, d)) for d in ["data","models","results"]):
        ROOT = _root; break
if ROOT is None:
    raise RuntimeError("Could not locate project root with data/models/results.")
...
def _find_best_threshold_acc_with_constraints(y_true, y_prob, min_f1=MIN_F1, min_prec=MIN_PREC):
    ...
    for t in thrs:
        ...
        if (m["f1"] >= min_f1) and (m["prec"] >= min_prec):
            candidates.append((m["acc"], t, m))
    if candidates:
        # max-acc; در تساوی، F1 بزرگتر و سپس Precision بزرگتر
        candidates.sort(key=lambda z: (z[0], z[2]["f1"], z[2]["prec"]), reverse=True)
        _, t, m = candidates[0]
        return t, m, True
    # fallback: بهترین F1 (اگر هیچ آستانه‌ای قیود را تامین نکرد)
    thrs_f1 = np.linspace(0.0, 1.0, 1001)
    best_f1 = None
    for t in thrs_f1:
        m = _metrics_at_threshold(y_true, y_prob, t)
        if (best_f1 is None) or (m["f1"] > best_f1[0]):
            best_f1 = (m["f1"], t, m)
...
def _run_training(mod, model_fname, cfg_overrides=None, seed=42):
    ...
    with open(os.path.join(results_subdir, "run_config.json"), "w") as f:
        json.dump({"model_file": model_fname, "cfg": cfg, "channels": CHANNELS,
                   "runner": "acc_prec_constrained", "min_f1": MIN_F1, "min_prec": MIN_PREC}, f, indent=2)
    ...
    for k in range(1, 6):
        ...
        # CONFIG["pos_weight"] can be: "auto" (default), None (disable -> 1.0), or float value
        ...
        # best acc with F1 & Precision constraints
        best_thr, m_best, satisfied = _find_best_threshold_acc_with_constraints(
            y_true_va, y_prob_va, min_f1=MIN_F1, min_prec=MIN_PREC
        )
        meta = {"best_threshold": float(best_thr),
                "constraint_f1_min": float(MIN_F1),
                "constraint_prec_min": float(MIN_PREC),
                "constraints_satisfied": bool(satisfied),
                "criterion": "maximize_accuracy_with_F1_and_Precision_constraints",
                "weight_decay": wd, "pos_weight": float(pos_weight.item())}
    ...
    with open(os.path.join(results_subdir, "_summary_avg_std.json"), "w") as f:
        json.dump({"avg": {k: float(v) for k,v in avg.items()},
                   ...
                   "note": f"metrics@best accuracy with F1 >= {MIN_F1} and Precision >= {MIN_PREC}"}, f, indent=2)
...
def main():
    ...
    parser.add_argument("--min_prec", type=float, default=None)
    args = parser.parse_args()
    global MIN_F1, MIN_PREC
