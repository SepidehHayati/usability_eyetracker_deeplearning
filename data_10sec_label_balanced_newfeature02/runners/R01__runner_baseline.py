# -*- coding: utf-8 -*-
"""
R01__runner_baseline.py
- 5-fold CV with train-only scaling
- EarlyStopping on F1 @ threshold=0.5
- Saves per-fold metrics/confusion/predictions and fold summaries
"""
...
@torch.no_grad()
def evaluate_fixed_threshold(model, loader, device, thr=0.5):
    """Evaluate at a fixed threshold (default 0.5)."""
    model.eval()
    y_true, y_prob = [], []
    for xb, yb in loader:
...
def _run_training(mod, model_fname, cfg_overrides=None, seed=42):
    ...
    for k in range(1, 6):
        ...
        for epoch in range(1, cfg["epochs"]+1):
            ...
            # quick validation @ thr=0.5 for monitoring / early stop
            val_res = evaluate_fixed_threshold(model, val_loader, device, thr=0.5)
            print(f"[fold {k}] epoch {epoch:02d} | loss={loss.item():.4f} | "
                  f"val: acc={val_res['acc']:.3f} f1={val_res['f1']:.3f} "
                  f"prec={val_res['prec']:.3f} rec={val_res['rec']:.3f}")
        ...
        # Save metrics
        metrics = {m: float(val_res[m]) for m in ["acc","prec","rec","f1"]}
        with open(os.path.join(fold_dir, "metrics.json"), "w") as f:
            json.dump(metrics, f, indent=2)

        # Confusion matrix
        plot_confusion(val_res["cm"], os.path.join(fold_dir, "confusion_matrix.png"))

        # Classification report
        with open(os.path.join(fold_dir, "classification_report.txt"), "w") as f:
            f.write(classification_report(val_res["y_true"], val_res["y_pred"], digits=4))

        # Predictions CSV
        pd.DataFrame({
            "y_true": val_res["y_true"],
            "y_pred": val_res["y_pred"],
            "y_prob": val_res["y_prob"],
        }).to_csv(os.path.join(fold_dir, "predictions.csv"), index=False)

        fold_metrics.append({"fold": k, **metrics})
    ...
    with open(os.path.join(results_subdir, "_summary_avg_std.json"), "w") as f:
        json.dump({"avg": {k: float(v) for k,v in avg.items()},
                   ...
                   "note": "metrics at fixed threshold=0.5"}, f, indent=2)
