# -*- coding: utf-8 -*-
# runners/R08_runner_acc_first.py
# آستانه با اولویت Accuracy + قیود F1/Precision (سخت→نرم→بدون قید)
...
class NpyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = self.X[idx]              # (T, C)
        x = np.transpose(x, (1,0))   # -> (C, T) برای Conv1d
...
def _choose_threshold_accfirst(y_true, y_prob,
    ...
    thrs = np.linspace(0.0, 1.0, 1001)  # گام ریزتر
    cand = []
    for t in thrs:
        ...
        if (m["f1"] >= hard_min_f1) and (m["prec"] >= hard_min_prec):
            cand.append(m)
    if len(cand) > 0:
        # بیشینه‌کردن Accuracy؛ در تساوی‌ها Precision بالاتر
    ...
    # هیچ‌کدام: بهترین Accuracy بدون قید
    best = None
    for t in thrs:
        m = _metrics_at_threshold(y_true, y_prob, t)
...
def run_with_model_path(model_path, epochs=None, batch_size=None, lr=None, patience=None, seed=42):
    ...
    with open(os.path.join(results_subdir, "run_config.json"), "w") as f:
        json.dump({"runner":"R08_acc_first",
                   "cfg": cfg,
                   "channels": CHANNELS,
    ...
    for k in range(1, 6):
        ...
        # آموزش + EarlyStop بر اساس F1@0.5 (ساده و پایدار)
        for epoch in range(1, cfg["epochs"]+1):
            ...
            if m05["f1"] > best_f1_05:
                ...
                patience_left -= 1
                if patience_left <= 0:
                    print(f"[fold {k}] early stopping.")
                    break

        # ارزیابی نهایی و انتخاب آستانه با Accuracy-first
        ...
        y_true_va, y_prob_va = _eval_probs(model, val_loader, device)
        thr, mb, hard_ok, soft_ok = _choose_threshold_accfirst(y_true_va, y_prob_va)
        ...
        pd.DataFrame({"y_true": y_true_va, "y_prob": y_prob_va, "y_pred_best": mb["y_pred"]}).to_csv(
    ...
    with open(os.path.join(results_subdir, "_summary_avg_std.json"), "w") as f:
        json.dump({"avg": {k: float(v) for k,v in avg.items()},
                   ...
                   "note": "metrics@best threshold (Accuracy-first with constraints)"}, f, indent=2)
    print("[DONE] Results in:", results_subdir)

# اجرای CLI مستقیم (اختیاری)
def main():
    p = argparse.ArgumentParser()
    p.add_argument("--model", required=True, help="مسیر فایل مدل (models/...) یا نامش")
    ...
    run_with_model_path(model_path, epochs=args.epochs, batch_size=args.batch_size,
