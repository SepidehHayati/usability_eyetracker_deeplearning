# -*- coding: utf-8 -*-
"""
runner_03_acc_constrained.py
- 5-fold CV with train-only scaling
- EarlyStopping on F1@0.5 (برای ثبات آموزش)
- Threshold tuning: maximize Accuracy subject to F1 >= MIN_F1 (per fold)
- Saves metrics at thr=0.5 and at best-acc-with-constraint, confusion matrices, predictions, and fold summaries
"""
...
# -------------------- Config for constraint --------------------
MIN_F1 = 0.75  # اگر خواستی سفت‌تر/شل‌تر کنی، این عدد رو عوض کن
...
def _find_best_threshold_acc_constrained(y_true, y_prob, min_f1=MIN_F1):
    # شبکهٔ دقیق‌تر چون مجموعه کوچک است
    thrs = np.linspace(0.0, 1.0, 1001)
    best = None   # (acc, thr, metrics)
    ...
    for t in thrs:
        ...
        if m["f1"] >= min_f1:
            if (best is None) or (m["acc"] > best[0]):
                best = (m["acc"], t, m)
    # اگر هیچ آستانه‌ای شرط F1 را پاس نکرد، به بهترین Acc بدون قید برگرد
    if best is None:
...
def _run_training(mod, model_fname, cfg_overrides=None, seed=42):
    ...
    with open(os.path.join(results_subdir, "run_config.json"), "w") as f:
        json.dump({"model_file": model_fname, "cfg": cfg, "channels": CHANNELS, "runner":"acc_constrained", "min_f1": MIN_F1}, f, indent=2)
    ...
    for k in range(1, 6):
        ...
        # best accuracy with F1 constraint
    ...
    # summary (metrics@best-acc-with-constraint)
    ...
    with open(os.path.join(results_subdir, "_summary_avg_std.json"), "w") as f:
        json.dump({"avg": {k: float(v) for k,v in avg.items()},
                   ...
                   "note": f"metrics@best accuracy with F1 >= {MIN_F1}"}, f, indent=2)
...
def main():
    ...
    parser.add_argument("--min_f1", type=float, default=None, help="Override MIN_F1 constraint at runtime")
