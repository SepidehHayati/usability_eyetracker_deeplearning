# 20_ensemble_accfirst_recall_plusf1.py
# Grid ensemble با قیود F1/Precision/Recall + بهبود fallback:
# - اگر هیچ آستانه‌ای قیود را پاس نکند: انتخاب آستانه = بیشینه‌ی F1 (تساوی → Acc بالاتر)
# - در انتخاب وزن‌ها: کلید مقایسه = (Acc, F1, Rec, Prec)
...
WEIGHT_RANGE = [0, 1, 2, 3, 4]          # وزن صفر مجاز
...
def choose_thr_accfirst_vec(y_true: np.ndarray, y_prob: np.ndarray, thrs: np.ndarray = THRS):
    ...
    if hard_idx.size:
        key = np.stack([acc[hard_idx], prec[hard_idx], -np.abs(thrs[hard_idx]-0.5)], axis=1)
        best_i = hard_idx[np.lexsort((key[:,2], key[:,1], key[:,0]))][-1]
        return dict(acc=float(acc[best_i]), prec=float(prec[best_i]), rec=float(rec[best_i]),
                    f1=float(f1[best_i]), thr=float(thrs[best_i])), True, False

    # 2) SOFT
    soft_idx = np.where((f1>=SOFT_MIN_F1) & (prec>=SOFT_MIN_PREC) & (rec>=SOFT_MIN_REC))[0]
    if soft_idx.size:
        key = np.stack([acc[soft_idx], prec[soft_idx], -np.abs(thrs[soft_idx]-0.5)], axis=1)
        best_i = soft_idx[np.lexsort((key[:,2], key[:,1], key[:,0]))][-1]
        return dict(acc=float(acc[best_i]), prec=float(prec[best_i]), rec=float(rec[best_i]),
                    f1=float(f1[best_i]), thr=float(thrs[best_i])), False, True

    # 3) Fallback: **بیشینه F1** (تساوی → Acc بالاتر)
    key_all = np.stack([f1, acc], axis=1)
...
def main():
    ...
    out_root = os.path.join(RESULTS_DIR, "20_ensemble_accfirst_recall_plusf1")
    os.makedirs(out_root, exist_ok=True)

    for k in range(1, 6):
        y_true, P = folds_data[k]  # P: (M, N)
        best_tuple = None  # (acc, f1, rec, prec, metrics_dict, hard_ok, soft_ok, weights, y_prob_ens)
        ...
        for w in weight_grids:
            ...
            # کلید انتخاب وزن: Acc → F1 → Rec → Prec
            key = (mb["acc"], mb["f1"], mb["rec"], mb["prec"])
            if (best_tuple is None) or (key > (best_tuple[0], best_tuple[1], best_tuple[2], best_tuple[3])):
                best_tuple = (mb["acc"], mb["f1"], mb["rec"], mb["prec"],
                              mb, hard_ok, soft_ok, w.copy(), y_prob_ens.copy())

        _, _, _, _, mb, hard_ok, soft_ok, w_best, y_prob_ens = best_tuple
    ...
    with open(os.path.join(out_root, "_summary_avg_std.json"), "w") as f:
        json.dump({
            ...
            "note": "Grid ensemble with constraints; fallback=max F1; weight key=(Acc,F1,Rec,Prec)",
            "models": MODEL_RESULTS,
