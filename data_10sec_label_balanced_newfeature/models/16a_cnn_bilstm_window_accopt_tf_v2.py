# 16a_cnn_bilstm_window_accopt_tf_v2.py
# CNN + BiLSTM with GAP+GMP pooling, tuned for higher file-level Accuracy
# windowing + standardize per-fold + class_weight (NEG_BOOST)
...
RESULT_DIR = os.path.join("..", "results", "16a_cnn_bilstm_window_accopt_tf_v2")
...
# ======================= Tuned Config =======================
WIN, STR        = 500, 200          # پنجره بلند + همپوشانی کافی (داده‌ی بیشتر نسبت به STR=250)
N_SPLITS        = 6
EPOCHS          = 100
BATCH_SIZE      = 16
LR_INIT         = 1e-3
...
DROPOUT_HEAD    = 0.35
RANDOM_STATE    = 42
VAL_FILE_RATIO  = 0.35              # نسبت ولیدیشن فایل متعادل‌تر از 0.45
THR_MIN, THR_MAX = 0.40, 0.80
THR_STEPS       = 41
SEEDS           = [7, 17, 23]
TOPK_K          = 5
NEG_BOOST       = 1.4               # کمتر از 1.6 برای جلوگیری از FP/کالیبراسیون بد
AGG_MODE        = "trimmed"         # قبلاً پایدارتر از median بود
...
# ======================= Model: CNN + BiLSTM (GAP+GMP) =======================
def build_model(input_shape):
    model = Sequential([
        tf.keras.Input(shape=input_shape),

        Conv1D(64,  kernel_size=7, padding='same', kernel_regularizer=regularizers.l2(L2_REG)),
        BatchNormalization(), Activation('relu'),
        MaxPooling1D(pool_size=2), Dropout(DROPOUT_BLOCK),

        Conv1D(96,  kernel_size=5, padding='same', kernel_regularizer=regularizers.l2(L2_REG)),
        BatchNormalization(), Activation('relu'),
        MaxPooling1D(pool_size=2), Dropout(DROPOUT_BLOCK),
        ...
        # GAP + GMP concatenation for robust pooling
        tf.keras.layers.Lambda(lambda x: Concatenate()([GlobalAveragePooling1D()(x),
