# 14d_cnn_window_ensemble_topk_accopt_tf.py
# هدف: بالا بردن Accuracy فایل‌سطح با (1) انتخاب آستانه برای بیشینه‌سازی دقت،
#      (2) تقویت وزن کلاس 0 (non-easy) در آموزش پنجره‌ها.
# پایه: 14c (windowing + seed-ensemble + Top-K + val stratified)
...
RESULT_DIR = os.path.join("..", "results", "14d_cnn_window_ensemble_topk_accopt_tf")
...
DROPOUT_HEAD    = 0.35
RANDOM_STATE    = 42
VAL_FILE_RATIO  = 0.35      # کمی بیشتر برای پایداری انتخاب آستانه
THR_MIN, THR_MAX = 0.35, 0.75
THR_STEPS       = 41
SEEDS           = [7, 17, 23]
TOPK_K          = 5
NEG_BOOST       = 1.4       # تقویت وزن کلاس 0 (non-easy). 1.2..1.6 را می‌توان تست کرد.
...
def choose_files_for_validation_stratified(train_file_ids, y_file_dict, ratio, seed=0):
    ...
    # سهم کلاس‌ها متناسب با نسبت واقعی
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    # انتخاب آستانه برای بیشینه‌سازی Accuracy روی فایل‌های ولیدیشن
...
summary = [
    f"WIN={WIN}, STR={STR}, Seeds={SEEDS}, TopK={TOPK_K}, NEG_BOOST={NEG_BOOST}",
