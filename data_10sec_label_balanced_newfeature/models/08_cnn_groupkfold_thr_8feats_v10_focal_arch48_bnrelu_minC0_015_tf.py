# 08_cnn_groupkfold_thr_8feats_v10_focal_arch48_bnrelu_minC0_015_tf.py
# CNN + StratifiedGroupKFold + per-fold thresholding + Seed-ensemble + Top-K snapshot averaging
# Changes vs v9:
# - Focal Loss (gamma=2.0, alpha=0.5) instead of BCE
# - LABEL_SMOOTH=0.0; class_weight OFF (handled by alpha in focal)
# - MIN_C0_RATIO=0.15 (less strict)
...
RESULT_DIR = os.path.join("..", "results", "08_cnn_groupkfold_thr_8feats_v10_focal_arch48_bnrelu_minC0_015_tf")
...
L2_REG            = 1e-5
FOCAL_GAMMA       = 2.0
FOCAL_ALPHA       = 0.5  # weight for positive class (label=1). Tune in [0.35..0.65].
...
# ===========================
@tf.function
def binary_focal_loss(y_true, y_pred, gamma=FOCAL_GAMMA, alpha=FOCAL_ALPHA):
    """
    y_true: shape (batch, 1) or (batch,)
    y_pred: predicted probability in [0,1], shape like y_true
    """
    ...
    loss = alpha_t * tf.pow(1. - pt, gamma) * ce
    return tf.reduce_mean(loss)

# ===========================
# Model (BN→ReLU)
# ===========================
def build_model(input_shape, lr=LR_INIT, l2_reg=L2_REG):
    model = Sequential([
        ...
        MaxPooling1D(pool_size=2), Dropout(0.30),
...
for tr_idx, te_idx in outer.split(X, Y, groups=G):
    ...
    # class weights OFF (focal handles imbalance via alpha)
    ...
    for seed in SEEDS:
        ...
        model = build_model((X_tr.shape[1], X_tr.shape[2]), lr=LR_INIT, l2_reg=L2_REG)
