# 16a_cnn_bilstm_window_accopt_tf.py
# Pipeline: windowing (WIN, STR) + standardize per fold + class_weight(NEG_BOOST)
# model: CNN feature extractor + BiLSTM + GAP + head
# seed-ensemble + TopK snapshot averaging
# file-level aggregation (AGG_MODE) + threshold chosen to maximize file-level Accuracy
...
RESULT_DIR = os.path.join("..", "results", "16a_cnn_bilstm_window_accopt_tf")
...
def build_model(input_shape):
    model = Sequential([
        ...
        # sequence modeling
        Bidirectional(LSTM(64, return_sequences=True, dropout=0.15, recurrent_dropout=0.0)),
        LayerNormalization(),
