# 14f_cnn_window_ensemble_hybridrule_accopt_tf_v2b_best.py
# Hybrid rule (fraction / count / quantile) + fallback
# Tuned for higher Accuracy:
# WIN=500, STR=200, AGG_MODE=trimmed, NEG_BOOST=1.4
# Softer grids for T_WIN / ALPHA / K / Q and wider file-threshold search
...
# ======================= Config (tuned) =======================
WIN, STR        = 500, 200          # more windows per file
N_SPLITS        = 6
EPOCHS          = 80
BATCH_SIZE      = 16
LR_INIT         = 1e-3
...
RANDOM_STATE    = 42
VAL_FILE_RATIO  = 0.40
THR_MIN, THR_MAX = 0.40, 0.80       # wider search for file-level threshold
THR_STEPS       = 41
SEEDS           = [7, 17, 23]
TOPK_K          = 5
NEG_BOOST       = 1.4               # lighter than 1.6 for Acc
AGG_MODE        = "trimmed"         # 'mean' / 'median' / 'trimmed'

# softer hybrid grids
TWIN_GRID   = np.linspace(0.45, 0.60, 4)          # 0.45, 0.50, 0.55, 0.60
ALPHA_GRID  = np.array([0.10, 0.15, 0.20, 0.25])  # fraction rule
K_GRID      = np.array([1, 2])                    # count rule
Q_GRID      = np.array([0.70, 0.80])              # quantile rule
...
def grid_search_hybrid_on_val(win_probs_val, files_val, y_file, thr_file):
    ...
    best = {"acc": -1.0, "mode": "fraction", "param1": 0.5, "param2": 0.2}
    # fraction
    for tw in TWIN_GRID:
        for a in ALPHA_GRID:
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    thr_file, acc_val_base = choose_threshold_for_accuracy(val_file_probs, val_file_true,
                                                           tmin=THR_MIN, tmax=THR_MAX, steps=THR_STEPS)
    print(f"[FOLD {fold}] thr_file={thr_file:.3f} | Acc(val-files) baseline={acc_val_base:.3f}")
    ...
    # decide function chosen (fallback if hybrid <= baseline on validation)
    def predict_file(win_probs):
        if best_h["acc"] >= acc_val_base:
    ...
    rows.append({"fold":fold,
                 ...
                 "acc_val_baseline": float(acc_val_base),
