# 16b_transformer_window_accopt_tf_fast_v4.py
# تغییرات نسبت به v3:
# - بازه‌ی آستانه: THR_MIN=0.55, THR_MAX=0.90
# - NEG_BOOST=1.6 (تقویت کلاس 0 برای کاهش FP)
# - VAL_FILE_RATIO=0.50 (ولیدیشن فایل بزرگ‌تر برای پایداری انتخاب آستانه)
# - قید انتخاب آستانه: حداقل share کلاس 0 در پیش‌بینی‌ها >= 0.30
# - همان معماری ترنسفورمر فست (1 لایه، d_model=64, heads=4, ffn=128)
...
RESULT_DIR = os.path.join("..", "results", "16b_transformer_window_accopt_tf_fast_v4")
...
WIN, STR        = 500, 200
N_SPLITS        = 6
EPOCHS          = 60           # کمی کمتر برای سرعت
BATCH_SIZE      = 16
LR_INIT         = 1e-3
L2_REG          = 1e-5
DROPOUT         = 0.20
RANDOM_STATE    = 42
VAL_FILE_RATIO  = 0.50         # ← بزرگ‌تر از قبل
THR_MIN, THR_MAX = 0.55, 0.90  # ← سفت‌تر
THR_STEPS       = 36
MIN_C0_RATIO    = 0.30         # ← قید: حداقل 30% پیش‌بینی‌ها صفر
SEEDS           = [7, 17, 23]
TOPK_K          = 5
NEG_BOOST       = 1.6          # ← تقویت کلاس 0
AGG_MODE        = "trimmed"    # trimmed یا median
TRIM_FRAC       = 0.10         # برای trimmed-mean، 10% دو سر حذف می‌شود
...
def choose_threshold_for_accuracy_with_constraint(val_probs_dict, val_true_dict,
    ...
    # ابتدا فقط آستانه‌هایی که حداقل سهم کلاس 0 را تامین می‌کنند
    for t in grid:
        y_hat = (p_vec > t).astype(int)
        if np.mean(y_hat == 0) < min_c0_ratio:
            continue
        acc = accuracy_score(y_true, y_hat)
        if acc > best_acc:
            best_acc, best_t = acc, t
    # اگر هیچ آستانه‌ای قید را پاس نکرد، fallback به بهترین دقت بدون قید
    if best_acc < 0:
        for t in grid:
            y_hat = (p_vec > t).astype(int)
            acc = accuracy_score(y_true, y_hat)
...
# ======================= Positional Encoding (fixed dtype) =======================
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    # انتخاب استراتیفای‌شده‌ی فایل‌های ولیدیشن (روی فایل)
    ...
    if use_platt:
        ...
        calib.fit(v_p, v_y)
        # اعمال کالیبراسیون روی ولیدیشن و تست
        ...
        # اگر فقط یک کلاس در ولیدیشن بود، بدون کالیبراسیون ادامه می‌دهیم
        val_file_probs_cal = val_file_probs
        te_file_probs_cal  = te_file_probs

    # ===== انتخاب آستانه با بیشینه‌سازی دقت + قید حداقل سهم کلاس 0 =====
    val_file_true = {int(f): int(Y[int(f)]) for f in val_file_probs_cal.keys()}
    thr, acc_val = choose_threshold_for_accuracy_with_constraint(val_file_probs_cal, val_file_true,
                                                                 tmin=THR_MIN, tmax=THR_MAX,
                                                                 steps=THR_STEPS,
                                                                 min_c0_ratio=MIN_C0_RATIO)
    print(f"[FOLD {fold}] Calibrated+ThrOpt: thr={thr:.3f} (val Acc={acc_val:.3f})")

    # ===== ارزیابی روی تست (فایل‌سطح) =====
...
summary = {
    ...
    "Config": {
        ...
        "MIN_C0_RATIO": MIN_C0_RATIO,
        "Transformer": {"d_model":64, "heads":4, "layers":1, "ffn":128, "dropout":DROPOUT}
    }
}
with open(os.path.join(RESULT_DIR, "summary.json"), "w", encoding="utf-8") as f:
    json.dump(summary, f, indent=2)

print("== Calibrated+ThrOpt (constrained) ==")
