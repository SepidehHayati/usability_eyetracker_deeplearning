# 18c_filelevel_weighted_ensemble_plus_meta_tf-best63.py
# اِنسمبل وزن‌دار + استکینگ (متافچر) برای بهبود Accuracy فایل‌سطح
...
RESULT_DIR = os.path.join("..", "results", "18c_filelevel_weighted_ensemble_plus_meta_tf_best63")
...
# ======================= Config =======================
# پنجره و stride مطابق بهترین‌های قبلی
WIN, STR        = 500, 200
N_SPLITS        = 6
EPOCHS          = 80
BATCH_SIZE      = 16
...
RANDOM_STATE    = 42
VAL_FILE_RATIO  = 0.35
AGG_MODE        = "median"       # median پایدارتر از trimmed در داده‌ی شما
AGG_TRIM_Q      = 0.10
THR_MIN, THR_MAX = 0.40, 0.80    # محدوده‌ی معقول برای Acc-opt
THR_STEPS       = 41
SEEDS           = [7, 17, 23]
TOPK_K          = 5
NEG_BOOST       = 1.4            # وزن‌دهی بیشتر به کلاس 0 برای کاهش FP
USE_PLATT_CAL   = True           # کالیبراسیون Platt روی فایل‌های ولیدیشن
# ترنسفورمر سریع
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    # متافچرهای فایل‌سطح از ۳ احتمال:
    ...
    # علاوه بر این، چند متافچر از توزیع پنجره‌ای (fraction>0.5 و IQR) برای هر مدل:
    ...
    # انتخاب روش برنده بر اساس Acc روی ولیدیشن (منطقی‌تر):
    # - ولیدیشن اِنسمبل وزن‌دار: best_val_acc_w
    # - ولیدیشن استکینگ: با CV کوچک روی v_files؟ ساده: score روی v_files
    # (برای سادگی همین‌جا با predict_proba روی v_files می‌سنجیم)
