# 17b_inceptiontime_window_accopt_tf.py
# InceptionTime + windowing + GroupKFold(user) + TopK snapshot
# + file-level median aggregation + accuracy-opt threshold
...
RESULT_DIR = os.path.join("..", "results", "17b_inceptiontime_window_accopt_tf")
...
def compute_class_weights(y, neg_boost=1.0):
    ...
    if 0 in w:  # افزایش جریمه FP
        w[0] = w[0] * neg_boost
    return w

def agg_file_level(probs, file_ids, mode=AGG_MODE, trim_q=AGG_TRIM_Q):
    ...
    for f, arr in d.items():
        ...
        if mode == "median":
            out[f] = float(np.median(a))
        elif mode == "trimmed":
            lo = np.quantile(a, trim_q); hi = np.quantile(a, 1-trim_q)
            m = a[(a>=lo) & (a<=hi)]
            out[f] = float(m.mean()) if len(m)>0 else float(a.mean())
        else:
            out[f] = float(a.mean())
    return out
...
# ======================= InceptionTime =======================
def inception_module(x, nb_filters=32, kernel_sizes=(10, 20, 40), bottleneck=32, l2=L2_REG, drop=DROP_RATE):
    # Bottleneck 1x1
    if int(x.shape[-1]) > 1:
        y = layers.Conv1D(bottleneck, 1, padding="same", activation=None,
    ...
    for k in kernel_sizes:
        c = layers.Conv1D(nb_filters, k, padding="same", activation=None,
                          kernel_regularizer=regularizers.l2(l2))(y)
        c = layers.BatchNormalization()(c)
        c = layers.Activation("relu")(c)
        convs.append(c)
    # pooling branch
    p = layers.MaxPooling1D(pool_size=3, strides=1, padding="same")(x)
    p = layers.Conv1D(nb_filters, 1, padding="same", activation=None,
                      kernel_regularizer=regularizers.l2(l2))(p)
    p = layers.BatchNormalization()(p)
    p = layers.Activation("relu")(p)
    convs.append(p)

    z = layers.Concatenate(axis=-1)(convs)
    z = layers.BatchNormalization()(z)
    z = layers.Activation("relu")(z)
    z = layers.SpatialDropout1D(drop)(z)
    return z

def shortcut_match_channels(x, y):
    if int(x.shape[-1]) != int(y.shape[-1]):
        x = layers.Conv1D(int(y.shape[-1]), 1, padding="same")(x)
    return layers.Add()([x, y])

def inception_block(x, nb_filters=32, kernel_sizes=(10,20,40), bottleneck=32, l2=L2_REG, drop=DROP_RATE):
    y = inception_module(x, nb_filters, kernel_sizes, bottleneck, l2, drop)
    return layers.Activation("relu")(shortcut_match_channels(x, y))
...
def build_inceptiontime(input_shape):
    ...
    for nf in [32, 48, 64]:
        x = inception_block(x, nb_filters=nf, kernel_sizes=(10,20,40), bottleneck=32)
        x = inception_block(x, nb_filters=nf, kernel_sizes=(10,20,40), bottleneck=32)
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    # file-level stratified validation split
    ...
    val_file_probs = agg_file_level(val_win_probs, Fva_in, mode=AGG_MODE, trim_q=AGG_TRIM_Q)
    te_file_probs  = agg_file_level(te_win_probs,  Fte,   mode=AGG_MODE, trim_q=AGG_TRIM_Q)

    val_file_true = {int(fi): int(Y[fi]) for fi in val_file_probs.keys()}
    te_file_true  = {int(fi): int(Y[fi]) for fi in te_file_probs.keys()}

    # accuracy-opt threshold
    thr, acc_val = choose_threshold_for_accuracy(val_file_probs, val_file_true,
