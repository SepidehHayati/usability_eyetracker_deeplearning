# 16b_transformer_window_accopt_tf_fast_v2.py
# Lite Transformer (balanced): d_model=64, heads=4, layers=1, ffn=128
# Fixes vs "fast":
# - softer NEG_BOOST, trimmed aggregation
# - wider threshold grid (0.40..0.80)
# - file-level Platt calibration fallback to Acc-opt
...
RESULT_DIR = os.path.join("..", "results", "16b_transformer_window_accopt_tf_fast_v2")
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    if not calibrated_used:
        # Acc-opt fallback
        val_true = {int(fi): int(Y[fi]) for fi in val_file_probs.keys()}
        thr_file, acc_val = choose_threshold_for_accuracy(val_file_probs, val_true,
...
cal = summarize(df, "calibrated"); accopt = summarize(df, "accopt")
if cal:    lines += ["== Calibrated ==", *cal]
if accopt: lines += ["== Acc-opt ==", *accopt]
