...
RESULT_DIR = os.path.join("..", "results", "06_cnn_groupkfold_thr_8feats_v7b_bestseed_bce_perfoldthr")
...
BATCH_SIZE        = 16
LR_INIT           = 1e-3
MIN_C0_RATIO      = 0.15           # relaxed vs 0.20
LABEL_SMOOTH      = 0.00           # off (can try 0.01 later)
TOPK_CHECKPOINTS  = 7              # average top-K snapshots per seed
USE_OVERSAMPLE    = False          # OFF now; rely on class_weight only
...
def best_threshold_with_constraint(val_probs, val_true, min_c0_ratio=MIN_C0_RATIO):
    ...
    # fallback: drop constraint if nothing found
    if best_macro < 0:
        for t in grid:
            yhat = (val_probs > t).astype(int)
            macro = f1_score(val_true, yhat, average='macro', zero_division=0)
...
def average_weights(weight_list):
    ...
    avg = [np.mean([w[i] for w in weight_list], axis=0) for i in range(len(weight_list[0]))]
    return avg
...
class TopKSaver(tf.keras.callbacks.Callback):
    ...
    def __init__(self, model, k=TOPK_CHECKPOINTS):
        ...
        self.k = k
        self.snapshots = []  # list of (val_loss, weights)

    def on_epoch_end(self, epoch, logs=None):
        vloss = float(logs.get('val_loss', np.inf))
        # store a deep copy of weights
        weights_copy = [w.copy() for w in self.model_ref.get_weights()]
        self.snapshots.append((vloss, weights_copy))

    def set_topk_weights(self):
        if len(self.snapshots) == 0:
            return
        # sort by val_loss asc
        snaps_sorted = sorted(self.snapshots, key=lambda x: x[0])
        top = snaps_sorted[:min(self.k, len(snaps_sorted))]
        weight_list = [w for (_vl, w) in top]
        avg_w = average_weights(weight_list)
        self.model_ref.set_weights(avg_w)

# ===========================
# Model
# ===========================
def build_model(input_shape, lr=LR_INIT, label_smooth=LABEL_SMOOTH, l2_reg=1e-5):
    model = Sequential([
...
# ===========================
# Training per fold (+seed-ensemble) with top-K averaging
# ===========================
...
for tr_idx, te_idx in outer.split(X, Y, groups=G):
    ...
    # inner split (group-aware) to get validation for threshold selection
    ...
    for seed in SEEDS:
        ...
        model = build_model((X_tr.shape[1], X_tr.shape[2]),
                            lr=LR_INIT, label_smooth=LABEL_SMOOTH, l2_reg=1e-5)
        ...
        # replace current weights with averaged top-K
        topk.set_topk_weights()

        # collect probs
        val_probs = model.predict(X_va, verbose=0).ravel()
        te_probs  = model.predict(X_te, verbose=0).ravel()
        val_prob_seeds.append(val_probs)
        te_prob_seeds.append(te_probs)
        seed_models.append(model)

    # average seeds' probabilities
    val_probs_mean = np.mean(val_prob_seeds, axis=0)
    te_probs_mean  = np.mean(te_prob_seeds,  axis=0)

    # per-fold threshold with constraint (on averaged validation probs)
    ...
    # evaluate on test for this fold
