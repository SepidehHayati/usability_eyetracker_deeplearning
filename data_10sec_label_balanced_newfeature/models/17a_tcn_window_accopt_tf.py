# 17a_tcn_window_accopt_tf.py
# TCN + windowing + GroupKFold(user) + TopK snapshot + file-level median + accuracy-opt threshold
...
RESULT_DIR = os.path.join("..", "results", "17a_tcn_window_accopt_tf")
...
TOPK_K          = 5
NEG_BOOST       = 1.4
AGG_MODE        = "median"   # "median" یا "trimmed"
AGG_TRIM_Q      = 0.10       # اگر AGG_MODE="trimmed" شد
...
# ======================= TCN Model =======================
def tcn_block(x, filters, kernel_size=5, dilation=1, dropout=DROPOUT_BLOCK, l2=L2_REG):
    # causal dilated conv
    y = layers.Conv1D(filters, kernel_size, padding="causal", dilation_rate=dilation,
                      activation=None,
...
def build_tcn(input_shape):
    ...
    x = layers.Conv1D(32, 3, padding="causal",
                      kernel_regularizer=regularizers.l2(L2_REG))(inp)

    # stack of dilated residual blocks
    for f in [48, 64, 64]:
        for d in [1, 2, 4, 8]:
            x = tcn_block(x, filters=f, kernel_size=5, dilation=d)
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    for seed in SEEDS:
        ...
        model = build_tcn((Xtr_in.shape[1], Xtr_in.shape[2]))
    ...
    # threshold by accuracy on validation files
