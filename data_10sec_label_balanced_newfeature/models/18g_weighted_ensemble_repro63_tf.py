# 18g_weighted_ensemble_repro63_tf.py
# بازتولید تنظیماتی که ~63% Accuracy داد:
# - سه مدل: CNN ساده، Transformer سبک، InceptionTime سبک
# - Windowing: WIN=500, STR=200
# - Aggregation: median (پایدارتر برای شما)
# - Platt calibration روی فایل‌های ولیدیشن
# - فقط GRID وزن‌ها (بدون local search)
# - انتخاب بین Weighted و Stacking-LR بر اساس Acc ولیدیشن هر فولد
# - THR_STEPS=41 (همون گرید درشت‌تر قبلی)
#
# خروجی: جدول per-fold + MEAN±STD + ذخیره metrics_file_level.csv و summary.json
...
from tensorflow.keras.layers import (Conv1D, BatchNormalization, Activation,
                                     ...
                                     Dense, Dropout, Input, Layer, Concatenate)
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
from tensorflow.keras.optimizers import Adam

# ---------------- Paths & I/O ----------------
DATA_DIR   = os.path.join("..", "data")
RESULT_DIR = os.path.join("..", "results", "18g_weighted_ensemble_repro63_tf")
...
# وزن‌های کاندید (بدون local search)
WEIGHT_CANDIDATES = [
...
def agg_probs_window_to_file(win_probs, file_ids, mode=AGG_MODE, trim_q=AGG_TRIM_Q):
    ...
    for f, ps in d.items():
        ...
        if mode=="median":
            ...
            lo, hi = np.quantile(arr, [trim_q, 1.0-trim_q])
            mask = (arr>=lo) & (arr<=hi)
...
def best_threshold_for_accuracy(p, y, tmin, tmax, steps):
...
# Transformer سبک — 1 لایه encoder ساده + positional encoding سینوسی جمعی
...
def build_transformer(input_shape, d_model=TRF_DMODEL, heads=TRF_HEADS, layers=TRF_LAYERS, ffn=TRF_FFN, dropout=TRF_DROPOUT):
    ...
    for _ in range(layers):
        ...
        x  = tf.keras.layers.LayerNormalization()(x + x1)
        x2 = Sequential([Dense(ffn, activation='relu'), Dropout(dropout), Dense(d_model)])(x)
        x  = tf.keras.layers.LayerNormalization()(x + x2)
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    # Platt calibration روی ولیدیشن → اعمال روی تست
    ...
    # آرایه‌های ولید/تست
    v_files = sorted(val_file_cnn.keys())
    t_files = sorted(te_file_cnn.keys())
    ...
    print(f"[FOLD {fold}] Weighted → wCNN={best_w[0]:.2f}, wTRF={best_w[1]:.2f}, wINC={best_w[2]:.2f} | thr={best_thr_w:.3f} | Acc(val)={best_val_acc_w:.3f}")

    # ---- (B) Stacking LR (meta-features) ----
    def build_meta_features(vc, vt, vi):
        Xm = np.stack([vc, vt, vi], axis=1)
        mean = Xm.mean(1); mx = Xm.max(1); mn = Xm.min(1); var = Xm.var(1)
    ...
    Xmeta_v = np.concatenate([
        build_meta_features(v_cnn, v_trf, v_inc),
        build_window_meta(Fva_in, val_cnn, val_trf, val_inc, v_files)
    ], axis=1)
    Xmeta_t = np.concatenate([
        build_meta_features(t_cnn, t_trf, t_inc),
        build_window_meta(Fte,    te_cnn,  te_trf,  te_inc,  t_files)
    ], axis=1)

    stack_lr = LogisticRegression(max_iter=2000)
    stack_lr.fit(Xmeta_v, v_y)
    p_stack_v = stack_lr.predict_proba(Xmeta_v)[:,1]
    thr_sv, acc_sv = best_threshold_for_accuracy(p_stack_v, v_y, THR_MIN, THR_MAX, THR_STEPS)

    p_stack_t = stack_lr.predict_proba(Xmeta_t)[:,1]
    y_pred_s = (p_stack_t > thr_sv).astype(int)
    ...
    print(f"[FOLD {fold}] Stack LR → thr={thr_sv:.3f} | Acc(val)={acc_sv:.3f}")

    # انتخابِ بهتر بر اساس Acc ولیدیشن
    ...
    if picked_mode=="weighted":
        ...
        rows.append({"fold":fold,"mode":"stack_lr","acc":acc_s,"precision":prec_s,"recall":rec_s,"f1":f1_s,
                     "thr":thr_sv,"w_cnn":np.nan,"w_trf":np.nan,"w_inc":np.nan})
    fold += 1
...
mean_all, std_all = df.mean(numeric_only=True), df.std(numeric_only=True)

summary = {
    "weighted": sum_weighted,
    "stack_lr": sum_stack,
    "overall_mean_std": {
        "acc": [float(mean_all["acc"]), float(std_all["acc"])],
        "precision": [float(mean_all["precision"]), float(std_all["precision"])],
        "recall": [float(mean_all["recall"]), float(std_all["recall"])],
        "f1": [float(mean_all["f1"]), float(std_all["f1"])],
    },
    "config": {
        ...
        "THR_RANGE":[THR_MIN,THR_MAX,THR_STEPS],"USE_PLATT_CAL":USE_PLATT_CAL,
...
# پرینت تمیز
print("\n[PER-FOLD RESULTS]")
print(f"{'fold':>4} {'mode':>9} {'acc':>6} {'precision':>10} {'recall':>7} {'f1':>6} {'thr':>6}    weights(cnn,trf,inc)")
for _, r in df.iterrows():
    w_str = f"({r['w_cnn']:.2f},{r['w_trf']:.2f},{r['w_inc']:.2f})" if r["mode"]=="weighted" else "-"
    print(f"{int(r['fold']):4d} {r['mode']:>9} {r['acc']:.3f} {r['precision']:.3f} {r['recall']:.3f} {r['f1']:.3f} {r['thr']:.3f}    {w_str}")

print("\n[MEAN ± STD (overall picked per fold)]")
print(f"Accuracy : {mean_all['acc']:.4f} ± {std_all['acc']:.4f}")
print(f"Precision: {mean_all['precision']:.4f} ± {std_all['precision']:.4f}")
print(f"Recall   : {mean_all['recall']:.4f} ± {std_all['recall']:.4f}")
print(f"F1       : {mean_all['f1']:.4f} ± {std_all['f1']:.4f}")
