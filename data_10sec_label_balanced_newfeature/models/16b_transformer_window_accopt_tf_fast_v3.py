# 16b_transformer_window_accopt_tf_fast_v3.py
# Lite Transformer + Platt calibration with THRESHOLD OPT on validation files
# Config defaults (like your best runs):
#   WIN=500, STR=200, SEEDS=[7,17,23], NEG_BOOST=1.4, AGG_MODE="trimmed",
#   THR grid = 0.40..0.80
...
RESULT_DIR = os.path.join("..", "results", "16b_transformer_window_accopt_tf_fast_v3")
...
LR_INIT         = 1e-3
RANDOM_STATE    = 42
VAL_FILE_RATIO  = 0.40              # می‌توانی 0.50 هم تست کنی
THR_MIN, THR_MAX = 0.40, 0.80
THR_STEPS       = 41
SEEDS           = [7, 17, 23]
TOPK_K          = 5
NEG_BOOST       = 1.4               # نرم، جلوگیری از collapse به کلاس 0
AGG_MODE        = "trimmed"         # در داده‌های تو متعادل‌تر از median عمل کرده
L2_REG          = 1e-5

# Transformer (balanced-lite)
D_MODEL         = 64
N_HEADS         = 4
FFN_DIM         = 128
N_LAYERS        = 1
...
def choose_threshold_for_accuracy_from_vector(y_true, p_vec, tmin=THR_MIN, tmax=THR_MAX, steps=THR_STEPS):
...
class PositionalEncoding(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        pos = tf.cast(tf.range(T), dtype)[:, None]
        div_term = tf.exp(
            tf.cast(tf.range(0, d_model, 2), dtype) * (-tf.math.log(10000.0) / tf.cast(d_model, dtype))
        )
        sin_part = tf.sin(pos * div_term)
        cos_part = tf.cos(pos * div_term)
        pe = tf.reshape(tf.stack([sin_part, cos_part], axis=-1), (T, -1))
...
for tr_idx, te_idx in gkf.split(Xw, Yw, groups=Gw):
    ...
    # تجمیع به سطح فایل
    val_file_probs = agg_probs_file_level(val_win_probs, Fva_in, AGG_MODE)
    te_file_probs  = agg_probs_file_level(te_win_probs,  Fte,   AGG_MODE)

    # ===== روش A: Platt calibration + ThrOpt روی ولیدیشن =====
    ...
    if len(np.unique(v_y)) == 2:
        try:
            ...
            calib.fit(v_p, v_y)
            # کالیبراسیون روی ولیدیشن و انتخاب آستانه با Acc-opt
            v_p_cal = calib.predict_proba(v_p)[:, 1]
            thr_cal, acc_val = choose_threshold_for_accuracy_from_vector(
                v_y, v_p_cal, tmin=THR_MIN, tmax=THR_MAX, steps=THR_STEPS
            )

            # اعمال روی تست
            t_files = sorted(te_file_probs.keys())
            t_p = np.array([te_file_probs[f] for f in t_files], dtype=float)[:, None]
            t_y = np.array([Y[f] for f in t_files], dtype=int)
            t_p_cal = calib.predict_proba(t_p)[:, 1]
            y_pred = (t_p_cal > thr_cal).astype(int)
            ...
            rows.append({"fold":fold,"mode":"calibrated_thropt","thr":float(thr_cal),
                         "acc":acc,"precision":prec,"recall":rec,"f1":f1})
            print(f"[FOLD {fold}] Calibrated+ThrOpt: thr={thr_cal:.3f} "
                  f"(val Acc={acc_val:.3f}) | Acc={acc:.3f} Prec={prec:.3f} Rec={rec:.3f} F1={f1:.3f}")
            used_calibrated = True
        except Exception as e:
            print(f"[FOLD {fold}] Calibration failed → fallback to Acc-opt. Err={e}")

    # ===== روش B: Acc-opt مستقیم روی احتمال‌های غیروکالیبره =====
    if not used_calibrated:
        files = sorted(val_file_probs.keys())
        y_true_val = np.array([Y[f] for f in files], dtype=int)
        p_vec_val  = np.array([val_file_probs[f] for f in files], dtype=float)

        thr_file, acc_val = choose_threshold_for_accuracy_from_vector(
            y_true_val, p_vec_val, tmin=THR_MIN, tmax=THR_MAX, steps=THR_STEPS
        )
        print(f"[FOLD {fold}] Acc-opt fallback: thr={thr_file:.3f} (val Acc={acc_val:.3f})")

        t_files = sorted(te_file_probs.keys())
        t_y = np.array([Y[f] for f in t_files], dtype=int)
        t_p = np.array([te_file_probs[f] for f in t_files], dtype=float)
        y_pred = (t_p > thr_file).astype(int)
        ...
        rows.append({"fold":fold,"mode":"accopt","thr":float(thr_file),
                     "acc":acc,"precision":prec,"recall":rec,"f1":f1})
...
cal = summarize(df, "calibrated_thropt"); accopt = summarize(df, "accopt")
if cal:    lines += ["== Calibrated+ThrOpt ==", *cal]
if accopt: lines += ["== Acc-opt (fallback) ==", *accopt]
