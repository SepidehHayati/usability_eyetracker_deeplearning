...
RESULT_DIR = os.path.join("..", "results", "25_cnn_cv_std_cw_thresh_macro_tensorflow")
...
def create_model(input_shape):
    model = Sequential([
        ...
        GlobalAveragePooling1D(),     # ← ساده‌تر و پایدارتر از Flatten
...
def standardize_fold(X_tr, X_va, X_te):
    # reshape به (N*T, C) برای استانداردسازی کانال‌ها
    Ntr, T, C = X_tr.shape
...
# =====================
# انتخاب آستانه بر اساس Macro-F1
# =====================
def choose_threshold_macro_f1(val_probs, val_y, grid=THRESH_GRID):
    best_t, best_macro = 0.5, -1.0
    for t in grid:
        yhat = (val_probs > t).astype(int)
        macro_f1 = f1_score(val_y, yhat, average='macro', zero_division=0)
        if macro_f1 > best_macro:
            best_macro, best_t = macro_f1, t
...
for tr_idx, te_idx in rskf.split(X, Y):
    ...
    # ولیدیشن داخلی از روی train فولد (stratified)
    ...
    # آموزش روی Train و اعتبارسنجی روی Val
    model.fit(
        X_tr, Y_tr,
        epochs=EPOCHS,
    ...
    # انتخاب آستانه با Macro-F1 روی Val
    val_probs = model.predict(X_va, verbose=0).ravel()
    best_t, best_macro = choose_threshold_macro_f1(val_probs, Y_va)

    # ارزیابی روی Test فولد با آستانه برگزیده
