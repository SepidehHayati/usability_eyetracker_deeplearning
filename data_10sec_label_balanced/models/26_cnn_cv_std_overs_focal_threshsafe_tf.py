...
RESULT_DIR = os.path.join("..", "results", "26_cnn_cv_std_overs_focal_threshsafe_tf")
...
# focal params: alpha کوچک → تاکید بیشتر روی کلاس 0 (negative)
FOCAL_ALPHA = 0.15
FOCAL_GAMMA = 2.0
...
# =====================
def focal_loss_from_logits(alpha=0.15, gamma=2.0):
    """
    Binary focal loss with logits.
    alpha اعمال روی کلاس مثبت (label=1)؛ برای تاکید بر کلاس 0، alpha را کوچک می‌گیریم.
    """
...
def create_model(input_shape):
    ...
    model.compile(optimizer=Adam(LR), loss=focal_loss_from_logits(FOCAL_ALPHA, FOCAL_GAMMA), metrics=['accuracy'])
    return model
...
# =====================
# Oversampling کلاس 0 در Train (duplication ساده تا نسبت 1:1)
# =====================
def oversample_minority(X_tr, Y_tr, minority_label=0):
    ...
    if n_min == 0 or n_min >= n_maj:
        return X_tr, Y_tr  # نیاز نیست
    # تعداد نمونه‌های موردنیاز برای 1:1
    n_need = n_maj - n_min
...
# =====================
# انتخاب آستانه امن (Macro-F1 + جلوگیری از تک‌کلاسه شدن روی Val)
# =====================
def choose_threshold_macro_f1_safe(val_probs, val_y, grid=THRESH_GRID):
    ...
    for t in grid:
        ...
        classes_pred = np.unique(yhat)
        macro_f1 = f1_score(val_y, yhat, average='macro', zero_division=0)
        # ترجیح: هم Macro-F1 بالا، هم پیش‌بینی‌ها دوکلاسه باشند
        two_class = (len(classes_pred) == 2)
        if two_class and macro_f1 > best_macro:
            best_macro = macro_f1
            best_t = t
            best_two_class = True
        elif (best_two_class is None) and (macro_f1 > best_macro):
            # اگر هیچ آستانهٔ دوکلاسه پیدا نشد، بهترین macro را نگه می‌داریم
            best_macro = macro_f1
            best_t = t
...
for tr_idx, te_idx in rskf.split(X, Y):
    ...
    X_tr, Y_tr = oversample_minority(X_tr, Y_tr, minority_label=0)

    # استانداردسازی per-fold بعد از oversample
    X_tr, X_va, X_te = standardize_fold(X_tr, X_va, X_te)

    # class_weight (با وجود oversample، می‌توان ضعیف‌تر هم کرد؛ فعلاً نگه می‌داریم)
    ...
    # آموزش (loss از logits؛ metric accuracy)
    model.fit(
        X_tr, Y_tr,
        epochs=EPOCHS,
    ...
    # انتخاب آستانه امن روی Val
    val_logits = model.predict(X_va, verbose=0).ravel()
    val_probs  = 1.0 / (1.0 + np.exp(-val_logits))  # sigmoid
    best_t, best_macro = choose_threshold_macro_f1_safe(val_probs, Y_va)
