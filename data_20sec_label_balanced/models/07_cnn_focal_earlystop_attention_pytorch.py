...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x):  # x: (B, T, C)
        ...
        weights = torch.softmax(weights, dim=1)  # softmax روی زمان
        attended = torch.sum(x * weights, dim=1)  # جمع وزن‌دار روی زمان
        return attended

# مدل CNN با Attention
class CNNWithAttention(nn.Module):
    def __init__(self):
        ...
        self.attn = AttentionLayer(input_dim=64)
...
# ذخیره نتایج
result_dir = os.path.join("..", "results", "07_cnn_focal_earlystop_attention_pytorch")
